...
- Deprecated API with drop-in replacement
...
**Default to ASK when uncertain.** Batch ASK findings into a single summary.

## Correctness (Non-Obvious)

- Race conditions — shared mutable state, check-then-act, TOCTOU
- Off-by-one — fencepost, inclusive vs. exclusive ranges, pagination
- Floating-point for money — use integers/decimals
- Ordering assumptions — async operations, event handlers, map iteration
- Type assertions lying about reality (`as any`, unsafe casts)

## Security (What Models Miss)
...
- Response body filtering — allowlists not blocklists for serialized fields
- CORS not `*` in production

## Data Integrity

- Migration backwards-compatible? (can old code run against new schema?)
- Data loss possible? (column drops, type narrowing, cascade deletes)
- Rollback path for this migration?
- Default values sensible for existing rows?
...
- What's the blast radius? (one user, one tenant, all users?)
...
- Safe to deploy on Friday? If not, why?
...
_Promoted from `.ai/rules/process/code-review.mdc` into this skill's references._
