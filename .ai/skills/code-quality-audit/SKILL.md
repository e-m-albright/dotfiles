...
description: Audit code against universal engineering principles with a graded rubric. Use when reviewing modules, PRs, or conducting periodic health checks; user says "audit this", "review this code", "grade this PR", "is this code healthy?", "code-quality-audit"; or asks for a structured report-card review with letter grades. Produces a report card with letter grades across fixed criteria plus flexible domain-specific observations.
allowed-tools: Read Grep Glob Bash(git:*) Bash(wc:*) Bash(rg:*)
---
...
**Not this skill?** For a pre-merge *bug hunt* over a specific diff/branch/PR (correctness, security, data-integrity, ops defects) with fix-first classification, use **`premerge-review`**. This skill grades *health*; `premerge-review` finds *defects about to land*.
...
- **U11 (Module Depth)**: 1.25x — high leverage when modules are libraries, frameworks, or core domain code
