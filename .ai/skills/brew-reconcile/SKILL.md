---
name: brew-reconcile
description: Compare installed Homebrew packages against `~/dotfiles/macos/brew.sh` source-of-truth and report drift in both directions. Use when user invokes /brew-reconcile, asks about Homebrew drift, wants an audit of installed vs declared packages, or says "what brew packages are stale", "what's missing", or "is my brew in sync with dotfiles?".
