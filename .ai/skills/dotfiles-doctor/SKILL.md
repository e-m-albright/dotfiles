...
description: Run `dotfiles doctor` to check that all tools, symlinks, and configs are installed, then interpret results and suggest fixes. Use when user reports missing tools, broken symlinks, or version mismatches; asks "is everything set up correctly?", "is my dev env healthy?", "why is X not working?"; or after running `dotfiles install` / `dotfiles update` to verify success.
---
