---
name: design-review
description: This repo owner's frontend/UI review checklist and judgment — visual hierarchy, consistency, the full states-and-edge-cases matrix (empty/loading/error/partial/overflow/offline), responsiveness, WCAG accessibility, interaction quality, plus an explicit "AI slop detection" list. Use when reviewing or critiquing UI a diff already produced and you want THIS checklist — "review this UI", "design review", "critique this component", "does this look AI-generated", "check accessibility on this", "what edge cases am I missing", "is this slop", "review the frontend changes". Complements the `impeccable` skill (which designs and builds interfaces); reach for this when you're auditing existing frontend code against a fixed review rubric rather than generating new design.
metadata:
...
Apply when reviewing frontend or UI code. Not every item applies to every change — focus on what the diff touches.
