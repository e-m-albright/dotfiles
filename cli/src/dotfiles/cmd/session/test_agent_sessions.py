...
def test_live_agents_reads_session_from_process_env() -> None:
    ...
    runner.script(
        ...
        stdout=_env(session="people", pwd="/home/dev/code/private/notes"),
    )
    runner.script(
        ("ps", "eww", "-p", "21697", "-o", "command="),
        stdout=_env(session="skills", pwd="/home/dev/dotfiles"),
    )
...
def test_live_agents_same_cwd_two_sessions_each_keep_their_own() -> None:
    ...
    runner.script(
        ...
        stdout=_env(session="dotfiles", pwd="/home/dev/dotfiles"),
    )
    runner.script(
...
def test_live_agents_no_session_env_is_unmatched() -> None:
    ...
    runner.script(
        ...
        stdout=_env(session=None, pwd="/home/dev/code/public"),
    )

    matched, unmatched = agents_by_session(live_agents(runner))

    assert matched == {}
    assert [(a.agent, a.cwd) for a in unmatched] == [("claude", "/home/dev/code/public")]
