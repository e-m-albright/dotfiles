...
Paseo agent sessions have host authority, including tools supplied by the daemon. They are not equivalent to Workbench's restricted terminal sessions, even if a provider process is sandboxed. Retired Lima provider entries must remain disabled; only explicitly unrestricted providers should be used. Existing sessions retain their launch-time tools, so changing provider configuration does not restrict an already-running session.
