...
### Visual / Element-Select AI Editors

> "Circle an element in the browser, prompt the AI, code changes apply." Two flavors: overlays on your own dev server (edits land in your repo) vs builder platforms that own the codebase.

**Overlay on your own codebase**

| Tool | What It Does | Why Watch |
|------|--------------|-----------|
| **Stagewise** | Browser toolbar; ships DOM + screenshot + metadata as context to Cursor/Copilot/Windsurf or its own agent | YC S25, ~6.5k stars; React/Next/Vue/Svelte/SvelteKit; free tier, ~€20/mo paid; pivoting toward Electron "developer browser" |
| **Cursor 2.0 browser preview** | Built-in browser, select element + prompt loop | Native to Cursor; pairs with Stagewise / Chrome DevTools MCP |
| **Windsurf browser preview** | Same shape as Cursor — element select + prompt | Cognition/Devin ecosystem |
| **Onlook** | Figma-like canvas over a React/Next app | Open source; sandboxed web container (no real middleware/env visibility) — prototyping over debugging |
| **Builder.io Visual Copilot** | Visual + prompt edits land as real PRs against your repo | Best when review hygiene / CI matters |
| **Frontman** | Open-source, deep framework integration | Less polish, more control |
| **Subframe** | Design-to-code with click-to-edit | React/Tailwind output |
| **Tempo** | Visual React editor with AI | Has select-and-prompt mode |
| **Magic Patterns** | Design-first visual editing of generated components | Earlier-stage |
| **Tidewave** | Phoenix/Rails/Django backends | Backend-stack equivalent; not relevant for SvelteKit |

**Builder platforms (AI owns the codebase)**

| Tool | What It Does | Why Watch |
|------|--------------|-----------|
| **Claude Design** (Anthropic Labs) | claude.ai/design; Draw mode (⌘1) to sketch on canvas + prompt; Opus 4.7 | Research preview (Pro/Max/Team/Enterprise); reads codebase/Figma/site to build a design system; prototyping only, hands off to Claude Code for production; eats tokens fast (~58% of Pro weekly quota per 2 sessions reported); no Figma export yet |
| **Lovable** | "Visual Edits" — click element, change text/color/sizing without burning credits | Strong greenfield; wants to own the project |
| **v0 (Vercel)** | "Design Mode" for Next.js + shadcn output | Cleanest code generation in this tier |
| **Bolt** | Same shape, prompt-to-app | StackBlitz container model |
| **Replit Visual Editor** | Click-to-edit in Replit projects | Reportedly strong |
| **Dazl** | Wix co-founder's prompt-to-app builder | Most full-featured visual editor of the prototyping crowd per recent comparisons |
| **Figma Make** | Figma's prompt-to-site with inline edits | Less mature than Lovable |
