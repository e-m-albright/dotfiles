...
- **Reliability is a convergent pillar, mostly already ratcheted.** Coverage measures test *quantity*; the missing *quality* signal is **mutation score** — do the tests actually catch an injected bug, or just execute the line? The per-language tool lives in each [language pack](../../ai/skills/converge/lang/) (`mutmut`, `cargo mutants`, `stryker`, `go-mutesting`); `just mutation` runs it. Like perf it's slow (one suite run per mutant) → manual/nightly, ratcheted as a floor. Combined, Reliability's convergent home is: the coverage floor (quantity) + the mutation-score floor (quality) + the already-ratcheted silent-catch (`broad-except`) and skipped-test suppression counts.
