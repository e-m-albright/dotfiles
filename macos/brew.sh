...
core_cli=(
    ...
    helix               # Postmodern modal terminal editor (Rust, batteries-included LSP/Treesitter — binary is `hx`)
...
system_cli=(
    ...
    duti                # Set default app for file types (used by file-associations.sh)
)
...
ide=(
    ...
    zed                    # Secondary: GPU-native Rust editor with ACP agent panel (eval — see docs/ides.md)
...
productivity=(
    ...
    syntax-highlight       # Quick Look: syntax highlighting for code, yaml, toml, json, cfg, etc. (sbarex)
)
...
# Fonts (Nerd-Font-patched variants ship glyphs for statuslines, prompts, devicons)
fonts=(
    font-fira-code-nerd-font           # Active terminal font (Ghostty primary)
    # font-jetbrains-mono-nerd-font    # Alternate; Ghostty's default-ish vibe with glyphs
    # font-maple-mono-nf               # Editorial-leaning, sharp letterforms
    # font-monaspace-nerd-font         # GitHub Monaspace — five variants, texture-healing
    # font-mononoki-nerd-font          # Humanist mono with character
    # font-symbols-only-nerd-font      # Glyph-only overlay (use as fallback for non-patched primary)
)
...
ai_cli=(
    ...
    # codex-app          # Disabled 2026-05-07: CLI-first, no need for desktop
    # -- Google --
    googleworkspace-cli  # Google Workspace CLI (Drive, Gmail, Calendar, Sheets, Admin)
    # jules              # Disabled 2026-05-07: cloud-only (jules.google.com), local CLI is just a launcher
...
# npm-only developer tools (Homebrew formula `wrangler` is the Erlang refactor
# tool, disabled upstream 2025-07-01 — Cloudflare ships theirs via npm only)
print_section "Developer Tools (npm)"
if command -v npm >/dev/null 2>&1; then
    if command -v wrangler >/dev/null 2>&1; then
        print_pkg_installed "wrangler"
    else
        print_pkg_installing "wrangler"
        if npm install -g wrangler 2>&1; then
            print_pkg_done "wrangler"
        else
            print_pkg_fail "wrangler"
        fi
    fi
else
    print_warn "npm not found — skipping wrangler (install Node.js first)"
fi
