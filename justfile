...
# Scrub caches by default. Authored artifacts require `just scrub --artifacts` or `just scrub all`.
[group('cleanup')]
scrub mode='caches':
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    if $do_caches; then
        rm -rf "{{repo}}/cli/.complexipy_cache" "{{repo}}/cli/.ruff_cache" "{{repo}}/cli/.pytest_cache"
