...
setup_mcp() {
    ...
    # Preserve sections of config.toml that we don't manage
    # We own: the header, project_doc_fallback_filenames, and [mcp_servers.*] blocks
    # Codex owns: [marketplaces.*], [projects.*], and anything else
    if [[ -f "$CONFIG_TOML" ]]; then
        cp "$CONFIG_TOML" "$CONFIG_TOML.bak"
    fi

    # Extract non-managed sections from existing config (marketplaces, projects, etc.)
    local preserved_sections=""
    if [[ -f "$CONFIG_TOML.bak" ]]; then
        # Keep all [section] blocks that aren't mcp_servers and aren't our header keys
    ...
    # Write config: our managed header + MCP, then preserved sections
...
CONFIG_EOF
    ...
    # Append preserved non-managed sections
