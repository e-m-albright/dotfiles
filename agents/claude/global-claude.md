...
Project-specific coding rules are inlined in the project's AGENTS.md (look for a "Project Coding Rules" section between fenced markers). If the project has a `.ai/rules/` directory but no inlined section, run `dotfiles agent migrate-rules-sync` to set up cross-harness rule delivery — do NOT load `.ai/rules/*.mdc` files yourself; that indirection is unreliable.
