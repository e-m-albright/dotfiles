...
Two-phase audit pattern lifted from a private project and made portable. **Phase 1** runs deterministic tools and writes structured JSON to `.ai/artifacts/audits/<topic>/<ts>/raw.json`. **Phase 2** is an LLM synthesis pass that reads `raw.json` and writes a human-readable `findings.md`.
