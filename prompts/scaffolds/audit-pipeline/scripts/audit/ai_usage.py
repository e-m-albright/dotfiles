...
def parse_frontmatter(text: str) -> tuple[dict[str, str] | None, str | None]:
    ...
    Handles flat key:value pairs and multi-line list values:
        globs:
          - "**/*.md"
          - "**/*.mdc"
    The block-style list is collapsed into the value of the preceding key (we
    don't actually use the list contents, just check that the key was set).
    """
    ...
    for raw in body.splitlines():
        ...
        if not line or line.lstrip().startswith("#"):
            continue
        # Continuation of a block-style list (indented "- value" line).
        if line.startswith((" ", "\t")) and line.lstrip().startswith("-"):
            if current_key is not None:
                out[current_key] = (
                    (out.get(current_key) or "").strip() + " " + line.strip()
                )
                continue
            # Indented list with no preceding key — malformed.
            return None, f"malformed line (orphan list item): {line!r}"
        if ":" not in line:
...
# Path patterns that should be ignored by the .ai/ validators. These are
# reference docs inside skills and similar prose-only files that don't
# (and shouldn't) carry SKILL-style frontmatter.
EXCLUDED_PATH_PARTS = ("references/",)
...
# Link strings that are obvious placeholders in template / example docs.
# These are intentionally-broken — the surrounding doc tells the reader
# they're placeholders. The validator should not flag them.
PLACEHOLDER_LINK_RE = re.compile(
    r"^("
    r"(?:[a-z][a-z_-]*/)?foo\.md"  # foo.md or references/foo.md (generic example)
    r"|URL_at_pinned_commit"  # rule-template URL marker
    r"|\./?src/[a-z][a-z_-]*/.*"  # example "./src/<feature>/..." paths
    r"|\.\./?src/[a-z][a-z_-]*/.*"
...
def check_dead_links(run_dir: Path) -> dict:
    ...
    for p in sorted(set(targets)):
        ...
        for m in LINK_RE.finditer(text):
            ...
            if url.startswith("#"):
                continue
            if PLACEHOLDER_LINK_RE.match(url):
                continue
