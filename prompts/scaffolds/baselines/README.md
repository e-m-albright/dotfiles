...
Code-health ratchet system lifted from a private project and made portable. Counts of anti-patterns and per-file line ceilings live in `baselines.json`. Pre-commit hook fails if any metric exceeds its ceiling. Improvements are locked in via `--auto-ratchet`.
