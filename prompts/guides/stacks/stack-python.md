...
| Linter/Formatter | **Ruff** | Black + isort + flake8 (three tools) |
| Type checker | **ty** (Astral) | mypy (slower), Pyright (Node dep) |
...
| Need | Pick | Avoid |
|------|------|-------|
| Web API | **FastAPI** | Flask (no async), Django (heavy) |
...
| HTTP client | **httpx** | requests (sync-only) |
...
| Notebooks | **Marimo** | Jupyter (reproducibility issues) |
| Docs | **MkDocs + Material** | Sphinx (complex) |

## What NOT to Install

| Tool | Why Skip |
|------|----------|
| Beartype | Pydantic handles runtime validation |
| Loguru | structlog is the pick — one logger |
| SQLModel | Maintenance mode |
| Alembic | Atlas is faster, better DX |
| mypy | ty is faster, same Astral ecosystem |
| Pyright | ty is faster, no Node.js dependency |
| Black/isort | Ruff does both |

## Version Requirements

Python >=3.14, Ruff target `py313` (until Ruff adds 3.14 support)
