...
Arguments:
  ...
  --with-agent-rules-sync Deploy scripts/sync-agent-rules.sh + lefthook fragment
                          (bakes .ai/rules/*.mdc into AGENTS.md + Cursor symlinks)
...
Examples:
  ...
  $(basename "$0") --with-agent-rules-sync typescript my-app
...
# ---- Optional: agent rules sync (cross-harness rule delivery) ----
if [[ "$WITH_AGENT_RULES_SYNC" == true ]]; then
    echo ""
    echo -e "${BLUE}Deploying agent rules sync...${NC}"
    sync_src="$PROMPTS_DIR/scaffolds/agent-rules-sync"
    mkdir -p scripts
    if [[ -f "scripts/sync-agent-rules.sh" ]] && [[ "$FORCE" != true ]]; then
        print_skip "scripts/sync-agent-rules.sh"
    else
        cp "$sync_src/scripts/sync-agent-rules.sh" scripts/sync-agent-rules.sh
        chmod +x scripts/sync-agent-rules.sh
        print_step "Deployed scripts/sync-agent-rules.sh"
    fi
    if [[ ! -f "lefthook.agent-rules.yml" ]]; then
        cp "$sync_src/lefthook.agent-rules.yml" lefthook.agent-rules.yml
        print_step "Deployed lefthook.agent-rules.yml (fragment — merge into lefthook.yml)"
    fi
    # Hint user to run sync + add markers
    print_info "Next:"
    print_info "  1. Add fenced markers to AGENTS.md where you want rules baked:"
    print_info "     <!-- BEGIN: project rules (auto-generated from .ai/rules/) -->"
    print_info "     <!-- END: project rules -->"
    print_info "  2. Run: ./scripts/sync-agent-rules.sh"
fi
