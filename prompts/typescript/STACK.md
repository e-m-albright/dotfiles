...
## Internationalization (add when needed)

> **Only for full-stack web apps.** Static sites and APIs rarely need i18n.

| Category | Choice | Why Not Alternatives |
|----------|--------|---------------------|
| **SvelteKit** | Paraglide JS 2.0 | i18next has runtime overhead. Paraglide is compile-time, tree-shakable, type-safe. Officially endorsed by the Svelte team (in Svelte CLI). |
| **Astro (Starlight)** | Built-in | Starlight has first-class i18n — locale directories, fallback content, 30+ UI languages, RTL support. No library needed. |
| **Astro (non-Starlight)** | Astro routing + Paraglide | Use Astro's built-in i18n routing for URLs + Paraglide's Vite plugin for type-safe translation strings. |

Paraglide compiles message files into tree-shakable JS functions — only messages actually used ship in the bundle. You get autocomplete on message keys and compile-time errors for missing parameters.

```typescript
// Generated by Paraglide — fully typed
import * as m from '$lib/paraglide/messages';

m.greeting({ name: 'Evan' }); // "Hello, Evan!"
```

**Alternatives (weaker picks)**

| Library | Approach | Status |
|---------|----------|--------|
| svelte-i18n | Runtime (Svelte stores) | Maintained but no type safety. |
| typesafe-i18n | Generator + runtime | Type-safe but not SvelteKit-specific. |
| sveltekit-i18n | Runtime (SSR-friendly) | Lightweight, no type safety. |
